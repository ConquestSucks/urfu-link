...
export const SubjectMembersModal = ({
  isOpen,
  onClose,
  members,
}: SubjectMembersModalProps) => {
  return (
    <Modal
      visible={isOpen}
      transparent={true}
    ...
    >
      ...
      >
        <Pressable
          ...
          className="bg-[#0B1225] border border-white/10 rounded-3xl overflow-hidden w-full max-w-[420px] max-h-[80vh]"
        >
          ...
          <ScrollView
            className="w-full"
            contentContainerClassName="px-8 py-6 gap-4"
            showsVerticalScrollIndicator={false}
          >
            {members.map((member) => (
              <View key={member.id} className="flex-row items-center gap-4">
                <View className="relative">
                  <Avatar size={44} src={member.avatarUrl} />
                  {member.isOnline && (
                    <View className="absolute bottom-0 right-0 w-3 h-3 bg-[#00D492] border-2 border-[#0B1225] rounded-full" />
                  )}
                </View>
