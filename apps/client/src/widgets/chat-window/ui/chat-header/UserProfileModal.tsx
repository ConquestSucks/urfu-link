...
import { BellSlashIcon, EnvelopeIcon, PhoneIcon, VideoCameraIcon, XIcon, } from "phosphor-react-native";
...
export const UserProfileModal = ({ isOpen, onClose, user, }: UserProfileModalProps) => {
    return (<Modal visible={isOpen} transparent={true} animationType="fade" onRequestClose={onClose}>
      <Pressable className="flex-1 bg-black/60 justify-center items-center px-4" onPress={onClose}>
        <Pressable onPress={(e) => e.stopPropagation()} className="bg-[#0B1225] border border-white/10 rounded-3xl overflow-hidden w-full max-w-[420px]">
...
const ActionBtn = ({ icon: Icon, label }: {
    ...
    <Text className="text-[11px] text-[#90A1B9] font-medium text-center" numberOfLines={1}>
      {label}
    </Text>
  </Pressable>);
