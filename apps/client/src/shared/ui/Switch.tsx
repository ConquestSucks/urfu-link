...
export const Switch = ({ value, onValueChange, disabled }: SwitchProps) => {
    ...
    const animatedTrackStyle = useAnimatedStyle(() => ({
        backgroundColor: interpolateColor(progress.value, [0, 1], ["#314158", "#2B7FFF"]),
    }));
    const animatedHandleStyle = useAnimatedStyle(() => ({
        transform: [{ translateX: progress.value * 24 }],
    }));
    return (<AnimatedPressable onPress={() => !disabled && onValueChange(!value)} disabled={disabled} style={[
            animatedTrackStyle,
            {
                width: 48,
                height: 24,
