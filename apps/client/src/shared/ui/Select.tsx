      ...
      >
        <Text
          className={`text-[15px] transition-colors duration-300 ${
            selectedOption ? "text-white" : "text-[#45556C]"
          }`}
        >
        ...
        <Animated.View style={chevronStyle}>
          <ChevronDown size={20} color={isOpen ? "#2B7FFF" : "#45556C"} />
        </Animated.View>
      </Pressable>

      <Modal visible={isOpen} transparent animationType="none">
        <Pressable
          className="flex-1 bg-transparent cursor-default"
          ...
          >
            <View className="bg-[#0B1225] border border-white/10 rounded-xl overflow-hidden max-h-60 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.5)]">
              <ScrollView bounces={false} showsVerticalScrollIndicator={false}>
                {loading ? (
                  <View className="px-4 py-5 items-center justify-center">
                  ...
                  options.map((option, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isSelected && <Check size={18} color="#2B7FFF" />}
                      </Pressable>
                    );
                  })
                )}
