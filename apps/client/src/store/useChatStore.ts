...
interface ChatState {
  ...
  isLoading: boolean;
  hasMore: boolean;
  /** Сколько страниц от начала (новейших) сообщений уже подгружено, минус 1 */
  pageIndex: number;
...
function sliceNewestFirst(
  all: readonly ChatMessageProps[],
  pageIndex: number,
): ChatMessageProps[] {
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  loadMessages: async (chatId, kind, reset = false) => {
    ...
    const shouldReset =
      reset || activeThreadKey !== threadKey || activeThreadKey === null;
  ...
  loadMore: async (chatId, kind) => {
    const { isLoading, hasMore, pageIndex, activeThreadKey, loadMessages } =
      get();
    const threadKey = buildChatThreadKey(kind, chatId);

    if (isLoading || !hasMore) return;
    if (activeThreadKey !== threadKey) return;
